...
1. در صفحه‌ی این ریپو در گیت‌هاب، روی دکمه‌ی سبز **Code** بزنید → **Download ZIP** (یا با `git clone` ریپو رو بگیرید).
2. اگه لازم بود، فایل زیپ دانلودشده رو استخراج کنید.
3. در کروم به آدرس `chrome://extensions` بروید.
4. گزینه‌ی **Developer mode** (بالا-راست صفحه) را روشن کنید.
5. روی **Load unpacked** کلیک کنید و همون پوشه‌ای که `manifest.json` توشه (ریشه‌ی ریپو) رو انتخاب کنید.
6. مطمئن شوید «Notion Chat Focus Toggles» در لیست اکستنشن‌ها ظاهر شده است.
